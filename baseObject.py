...
class baseObject:
    ...
    def truncate(self):
        ...
        self.cur.execute(sql)
    def getByField(self,field,val,sort_by = None):
        if sort_by == None or sort_by not in self.fields:
            sql = f"Select * from `{self.tn}` where `{field}` = %s" 
        else:
            sql = f"Select * from `{self.tn}` where `{field}` = %s order by `{sort_by}` desc" 
        # print(sql,val)
        self.cur.execute(sql,(val))
        self.data = []
        for row in self.cur:
            self.data.append(row)
    def getByMultiField(self, fields, vals):
        sql = f"SELECT * FROM `{self.tn}` WHERE"
        condition = ""
        for field in fields:
            condition += f" `{field}` = %s AND"
        sql += condition[0:-4]
        self.cur.execute(sql,(vals))
        self.data = []
        for row in self.cur:
